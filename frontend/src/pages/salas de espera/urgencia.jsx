  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Error fetching urgent consultations data");
        }

        const consultasData = await response.json();

        // Filtrar las consultas que son de tipo "URGENCIA"
        const urgenciaConsultas = consultasData.filter(
          consulta => consulta.tipo_consulta === "URGENCIA"
        );

        setUrgenciaData(urgenciaConsultas);
      } catch (error) {
          ...
          <div className="row mb-2 mx-auto col-md-11 mt-4">
            <div className="col-sm-6">
              <h5 className="mb-0 text-gray-800">
                Lista de Consultas de Urgencia
              </h5>
            </div>
            ...
            <div className="col-md-12">
              <div className="table-responsive">
                <table className="table table-bordered" id="mydatatable">
                  ...
                  <tbody>
                    {urgenciaData.map((consulta, index) => (
                      <tr key={index}>
